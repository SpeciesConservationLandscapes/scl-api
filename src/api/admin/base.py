...
class CountryListFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        ...
        for o in model_admin.model.objects.all():
            countries.extend(o.countries)
        countries = set(countries)
        return [(c.code, format_html('<img src=\'{}\'> {}'.format(c.flag, c.name))) for c in countries]
...
class ProfileAdmin(BaseAdmin):
    list_display = ('first_name', 'last_name', 'email', 'country_list')
    search_fields = ['first_name', 'last_name', 'email', ]
    list_filter = (CountryListFilter,)

    def country_list(self, obj):
        return format_html(
            ", ".join(['<img src="{}"> {}'.format(c.flag, c.name) for c in obj.countries])
        )
